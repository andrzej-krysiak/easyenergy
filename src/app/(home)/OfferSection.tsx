...
const OfferSection = () => {
    return (
        <section className="bg-white relative flex flex-col justify-center items-center py-12 sm:py-16 lg:py-20 w-full overflow-hidden rounded-t-[3rem] sm:rounded-t-[4rem] lg:rounded-t-[5rem] shadow-[0_-15px_40px_-20px_rgba(0,0,0,0.1)] z-30">
                        ...
                        <ScrollReveal key={catIdx} delay={catIdx === 0 ? "delay-0" : catIdx === 1 ? "lg:delay-150" : "lg:delay-300"}>
                            ...
                            <div className="flex flex-col gap-3">
                                {category.items.map((item, itemIdx) => (
                                    <div 
                                        key={itemIdx}
                                        className="group relative flex flex-col p-5 bg-slate-50/50 hover:bg-white border border-slate-100 lg:hover:border-[#3385d9]/30 rounded-2xl lg:transition-all lg:duration-500 shadow-sm lg:hover:shadow-[0_20px_50px_-10px_rgba(51,133,217,0.15)] lg:hover:-translate-y-1 overflow-hidden cursor-default transform-gpu"
                                    >
                                        <div className="flex items-center gap-4 relative z-10 lg:transition-transform lg:duration-500 transform-gpu lg:group-hover:-translate-y-0.5">
                                            <div className="size-10 rounded-xl bg-white text-[#3385d9] flex items-center justify-center shadow-sm group-hover:bg-[#3385d9] group-hover:text-white transition-all duration-500 shrink-0 transform-gpu">
                                                <item.icon className="w-5 h-5" />
                                            </div>
                                            <div className="flex flex-col">
                                                <h5 className="font-bold text-slate-900 group-hover:text-[#3385d9] transition-colors duration-500">
                                                    {item.title}
                                                </h5>
                                                {/* Desktop: Animowany zanik, Mobile: Stały opis pomocniczy lub szybki fade */}
                                                ...
                                                <p className="block lg:hidden text-xs text-slate-400 font-medium group-hover:hidden opacity-100 group-hover:opacity-0 transition-opacity duration-200">
                                                    {item.short}
                                                </p>
                                            </div>
                                            <div className="ml-auto opacity-40 group-hover:opacity-100 transition-opacity">
                                                <FiPlus className="w-4 h-4 text-[#3385d9] group-hover:rotate-45 transition-transform duration-500 transform-gpu" />
                                            </div>
                                        </div>
                                        
                                        {/* Kontener expandera - Desktop: Grid Animation, Mobile: Simple fade-in */}
                                        ...
                                        {/* Mobile-only static expand (no layout animation) */}
                                        <div className="block lg:hidden h-0 group-hover:h-auto opacity-0 group-hover:opacity-100 transition-opacity duration-300 overflow-hidden pointer-events-none group-hover:pointer-events-auto">
                                            <p className="text-sm text-slate-600 pt-4 leading-relaxed font-medium">
                                                {item.desc}
                                            </p>
                                            <div className="pt-4 flex items-center relative z-20">
                                        ...
                                        <div className="absolute inset-0 bg-gradient-to-r from-[#3385d9]/[0.03] to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
                                    </div>
                                ))}
                            </div>
                        </div>
